...
function display() {
    ...
    boardgames.forEach(x => {
        document.getElementById('resultarea').innerHTML += "<tr><td>" + x.boardGameId + "</td><td>" + x.title + "</td><td>" + x.type + "</td><td>" + `<button type="button" onclick="remove(${x.boardGameId})">Delete</button>` + `<button type="button" onclick="showupdate(${x.boardGameId})">Update</button>` + "</td></tr>"
        console.log(x.title);
    });
}
